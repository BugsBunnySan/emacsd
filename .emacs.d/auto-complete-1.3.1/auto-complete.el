...
(defun ac-up-down-show ()
  "return t unless the text at point is the
   completed only ac candidate"
  (let ((ac-num-candidates (length ac-candidates)))
    (or (> ac-num-candidates 1)
	(and (= ac-num-candidates 1)
	     (not (string-equal ac-prefix (car ac-candidates)))))))

(defun ac-next ()
  "Select next candidate."
  (interactive)
  (if (and (ac-menu-live-p)
	   (ac-up-down-show))
      (progn
	(popup-next ac-menu)
	(setq ac-show-menu t)
    ...
    (let* ((info (ac-prefix requires ac-ignoring-prefix-def))
      ...
      (if (or (null point)
        ...
        (setq ac-show-menu (or ac-show-menu (if (eq ac-auto-show-menu t) t))
              ...
              ac-triggered t
              ac-current-prefix-def prefix-def)3
